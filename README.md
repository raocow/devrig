...
| `ghswitch` | On every `cd`, switches the GitHub CLI's (`gh`) active account to match whichever [account](#accounts) is bound to the directory you're in (needs `--gh-user` set on that account — see below). `gh`'s active account is a single setting shared by *every* terminal and session on the machine, so anything else using a different identity anywhere can silently flip it out from under you; this corrects it each time you `cd`, instead of leaving you to hit a confusing "Unauthorized" error and fix it by hand. Reads `gh`'s own local config directly (no network call), so a plain `cd` elsewhere costs nothing. **Not included in `devrig enable` (bare or `all`)** — unlike the other three, it's global cross-session state, not a local shell convenience, so it's opt-in by name: `devrig enable ghswitch`. **Unbound directories bind themselves**, on evidence rather than a guess: the accounts differ precisely in which repos they can reach, so it checks which of yours can actually *push* to this repo and binds that one, printing what it did and how to change it. Push access specifically — on a public repo every account gets READ, so a mere visibility check would match the wrong account; a public repo you don't own is never auto-bound. Nothing happens outside a GitHub repo or when no account has push access. That probe costs a network call, so it's gated: unbound directories only, once per directory per shell session. |
